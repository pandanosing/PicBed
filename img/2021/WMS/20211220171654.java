...
public class ExcelUtil<T>
{
    ...
    public void init(List<T> list, String sheetName, Type type, boolean isPdfExport)
    {
        if (list == null)
        {
    ...
    public List<T> importExcel(String sheetName, InputStream is) throws Exception
    {
        ...
        try{
            ...
            throw new BusinessException("文件解析异常，请检查文件是否解密！");
        }
    ...
    {
        this.init(list, sheetName, Type.EXPORT,false);
        return exportExcel();
    }
     ...
     * @return 结果
     */
    public AjaxResult exportExcel2Pdf(List<T> list, String sheetName)
    {
    ...
    {
        this.init(null, sheetName, Type.IMPORT, false);
        return exportExcel();
    }
    ...
    public Cell addCell(Excel attr, Row row, T vo, Field field, int column)
    {
        ...
        try
        {
            ...
            if (attr.isExport())
            {
                ...
                // 如果是自动列高
                if(isPdfExport){
                    short autoHeight = calcAndSetRowHeight(row, column, Convert.toStr(value));
                    // 要设置高度大于当前行高时
                    if(autoHeight > row.getHeight()){
                        // 填充好值后设置高度
                        row.setHeight(autoHeight);
                    }
                }
            }
        }
     ...
     * @param row
     */
    public short calcAndSetRowHeight(Row sourceRow, int cellIndex, String cellContent) {
        ...
        // System.out.println(cellIndex+" 单元格的宽度 : " + cellWidth + "    单元格的高度 : " + maxHeight + ",    单元格的内容 : " + cellContent);

        //字体的高度
        short fontHeight =(short) 10 * 20;
        //cell内容字符串总宽度
...
//                }
            ...
            if(isPartOfRowsRegion){
                ...
                for (int i = firstRow; i <= lastRow; i++) {
                    maxHeight = sourceRow.getSheet().getRow(i).getHeight() + addHeight;
                }
            }
        }
        // System.out.println(cellIndex+" 字体高度 : " + fontHeight + ",    字符串宽度 : " + cellContentWidth + ",    字符串需要的行数 : " + stringNeedsRows + ",   需要的高度 : " + stringNeedsHeight + ",   现在的行高 : " + maxHeight);
        // System.out.println("-------------------");

        return (short)maxHeight;
    }
